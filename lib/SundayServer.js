...
class SundayServer {
  constructor(logger, config, ssl) {
    ...
    if(!ssl){
      logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Sunday Server.js ssl- disabled', color:null}],0);
      self.server = net.Server();
    }
    else{
      // SSL connection. 4243
      logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Sunday Server.js ssl-enabled', color:null}],0);
      self.server = tls.Server();
    }
    //self.logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Sunday Server.js socket_server.address '+socket_server.address(), color:null}],0);

    // parse the config file "drink_config.js"
    self.error_codes = config.error_codes;

    self.machine_aliases = config.machine_codes; //machine aliases
    self.machine_ip_mapping = config.machine_ip_mapping; //machine IP mapping
    self.sunday_config = config.sunday; //sunday server config file
    self.machine_server = null;
    self.sunday_opcodes = config.sunday_opcodes;
    logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Sunday Server.js Constructor', color:null}],0);
  }//Constructor
  is_valid_opcode(){
    var self = this;
    //self.logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Sunday Server.js is_valid_opcode '+opcode, color:null}],0);
    //self.logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Sunday Server.js is_valid_opcode opcodes.length '+self.sunday_opcodes.length, color:null}],0);
    for(var i = 0; i < self.sunday_opcodes.length; i++){
      if(opcode == self.sunday_opcodes[i]){
        //self.logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Sunday Server.js is_valid_opcode return true '+opcode, color:null}],0);
        return true;
      }
    }
    //self.logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Sunday Server.js is_valid_opcode return false '+opcode, color:null}],0);
    return false;
  }//is valid opcode
  sunday_time(conn){
    var self = this;
  ...
  init(){
    var self = this;
    self.logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Sunday Server.js start', color:null}],0);

    // setup the server
    self.server.listen(self.sunday_config.port);
    self.logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Sunday Server.js '+self.server.address().address+":"+self.server.address().port, color:null}],0);
    self.logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Sunday Server.js Server Listening: '+ self.server.listening +' '+self.sunday_config.host+':'+self.sunday_config.port, color:null}],0);

    self.server.on('connection', function(socket){
      self.setup_connection_handler(socket);
      self.logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Sunday Server.js init connection '+socket.remoteAddress, color:null}],0);

      self.logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Sunday Server.js init connection '+socket.localPort +" "+socket.localAddress, color:null}],0);
    });
  }//init()
  set_machine_server(machine_server){
    var self = this;
    self.machine_server = machine_server;
    self.logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Sunday Server.js set_Machine_server', color:null}],0);
  }//set_machine_server
  setup_connection_handler(socket){
    var self = this;
    self.logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Sunday Server.js setup_connection_handler start', color:null}],0);
    ...
    socket.on('data', function(data){
      ...
      self.logger.log([{msg: self.sunday_time(conn), color: 'cyan'}, {msg: ' -  str_data'+str_data, color: null}], 0);

      if(tokenized_data.length > 0){
        tokenized_data[0] = tokenized_data[0].toUpperCase();
        self.logger.log([{msg: self.sunday_time(conn), color: 'cyan'}, {msg: ' - tokenized_data[0] '+tokenized_data[0], color: null}], 0);
        if(self.is_valid_opcode(tokenized_data[0])){
          ...
          self.logger.log([{msg: self.sunday_time(conn), color: 'cyan'}, {msg: ' -  SENT 415', color: null}], 0);
        }
      } else {
        self.send_msg_code('415', socket);
      }
    ...
    socket.on('close', function(data){
      ...
      self.logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Client socket closed', color:null}],0);
    ...
    socket.on('timeout', function(data){
      ...
      self.logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Client socket timeout', color:null}],0);
    });

    socket.on('error', function(data){
      // TODO - Make this pretty
      //console.log("Client socket error");
      self.logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Client socket error', color:null}],0);
    });
  }//setup_connection_handler
  send_msg_code(code, socket, message){
    var self = this;
    self.logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Sunday Server.js send_msg_code', color:null}],0);
  ...
  send_json_msg(socket, payload){
    var self = this;
    self.logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Sunday Server.js is_valid_opcode send_json_msg', color:null}],0);
  ...
  *************************************************************************/
  SERVERSTAT(command, socket, conn){
    var self = this;
  ...
  UPTIME(command, socket, conn){
    var self = this;

    self.send_msg_code('OK', socket, 'Up since: ' + self.time_started);
  }//UPTIME
  SENDCREDITS(command, socket, conn){
    var self = this;
  ...
  QUIT(command, socket, conn){
    socket.end("Good Bye\r\n");
  }//QUIT
  MACHINE(command, socket, conn){
    var self = this;
  ...
  GETBALANCE(command, socket, conn){
    var self = this;
    if(conn.authenticated != true){
      self.send_msg_code('204', socket);
      return;
