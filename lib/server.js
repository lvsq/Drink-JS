...
var sunday = new SundayServer(sunday_serv, logger, config, config.sunday);/*sunday: {host: '0.0.0.0',port: 4242},*/
logger.log([{msg: utils.get_time(), color: 'green'},{msg: ' - Server.js Sunday Server pass', color:null}],0);
// SSL connection. 4243
...
logger.log([{msg: utils.get_time(), color: 'green'},{msg: ' - Server.js Sunday Server set machine server pass', color:null}],0);

// 3). Init the machine server
machine_server.init();
logger.log([{msg: utils.get_time(), color: 'green'},{msg: ' - Server.js Machine Server Init pass', color:null}],0);

// 4). init the sunday server
sunday.init();
logger.log([{msg: utils.get_time(), color: 'green'},{msg: ' - Server.js Sunday Init pass', color:null}],0);
