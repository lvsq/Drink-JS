...
SundayServer2.prototype.init = function(){
  ...
  // setup the server
  this.server = net.createServer(function(socket){this.setup_connection_handler(socket);}).listen(this.sunday_config.port);
...
SundayServer2.prototype.is_valid_opcode = function(opcode){
  this.logger.log([{msg: utils.get_time(), color: 'red'},{msg: ' - Sunday Server.js is_valid_opcode '+opcode, color:null}],0);
